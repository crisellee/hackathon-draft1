...
class ConcernService {
  ...
  Future<void> submitConcern(Concern concern) async {
    ...
    try {
      ...
      await _logAction(
        ...
        action: 'SUBMITTED',
        details: 'Concern submitted in category: ${concern.category.name.toUpperCase()} for department: ${concern.department}',
      );
  ...
  Future<void> _executeAutoRouting(Concern concern) async {
    // If the student already chose a department, we use it, otherwise auto-route.
    String targetDept = concern.department.isNotEmpty ? concern.department : 'GENERAL ADMINISTRATION';
    
    if (concern.department.isEmpty) {
    ...
    try {
      ...
      await _logAction(
        ...
        actorId: 'SYSTEM',
        action: 'ROUTED',
        details: 'Routed to $targetDept based on category or selection.',
      );
    } catch (e) {
